...
$sheet_url = 'https://docs.google.com/feeds/download/spreadsheets/Export?key=1haUVaFIxFq5IPqZugJ8cfCEqBrZvFFzcA-uXB4pTfW4&exportFormat=csv&gid=0'
$output_dir = './routes'
...
rows.each do |row|
  ...
  data = Hash[data.map {|k,v| [k, v.to_f.round(2)]}]
  ...
  good[:station_dist] = if matches = dist.match(/(\d+)\s*Ly/i)
    (matches[0].to_f * 31556926).round 2
...
def found_route(route, depth)
  ...
  $p_output = []
  status = sprintf('Route #%d (warnings=%d, jumps=%d, distance=%.2f, %s)',
                   $routes.length + 1, warnings.length, route.length,
                   route_distance, $version_info)
  status += ' PERFECT' if warnings.empty?
  puts status
  p status; p
  p "#{route}"; p
  unless warnings.empty?
    warnings.each {|obj| p "[Warn] #{obj[:message]}"}; p
  end
  ...
  route.each_with_index do |system_name, index|
    ...
    sell_goods_at[system_name].each do |source|
      ...
      $systems[source][:goods].each do |good|
        p "  SELL #{good[:name]} (#{source} @ #{distance} Ly)"
      end
    ...
    system[:goods].each do |good|
      ...
      p "   BUY #{name} x #{cap} (#{station_name} @ #{station_dist} Ls)"
    end
    target = route[index + 1] || route.first
    distance = system[:distance][target]
    p "  NEXT #{target} (#{distance} Ly)"
  end; p

  filename = "#{warnings.empty? ? 'perfect' : "warnings-#{warning_weight}"}-jumps-#{route.length}.txt"
...
def all_done
  ...
  unless routes.empty?
    filename = "best-#{$best_count}.txt"
    best_routes = routes[0..$best_count-1].map.with_index(1) do |route, index|
      header = "[ BEST ROUTE ##{index} ]"
      header = "#{'=' * ((width - header.length) / 2)}#{header}"
      header = "#{header}#{'=' * (width - header.length)}"
      ['', header, ''] + route[:output]
    end.flatten
    write_to_file filename, [banner, ''] + best_routes + ['']
  else
